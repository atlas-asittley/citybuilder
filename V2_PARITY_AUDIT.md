...
- [x] **Collector walker road pathing** (`v1-parity audit, commit 58/N`) — `MainScene.js _spawnCollectorWalker`. v1 uses weighted Dijkstra (road=1, off-road=3) so collectors visibly traverse the road network. v2 walks a straight line through grass.
...
- [x] **Black Market: inline quantity stepper** (`v1-parity audit, commit 56/N`) — `TradeBlackMarketTab.js`. v1 has -, qty, + buttons before commit. v2 uses `prompt()` modal.
...
- [x] **Build menu: housing tier evolution hint** (`v1-parity audit, commit 54/N`) — `BuildTabPanel.js describeBuilding`. v1: "Shanty → Mud Hut → ... → Palace. Workers 2–100. Prereqs: T1 well, T2 food, ...". v2: "Citizens live here. Upgrades unlock as you provide services + food."
...
- [x] **Inspector: service input requirements with period clarity** (`v1-parity audit, commit 53/N`) — `InspectorPanel.js`. v1: "Consumes 2 lumber + 2 flour per 2 min" (integer recipe). v2: rate only.
- [x] **Inspector: refund amount pre-demolish** (`v1-parity audit, commit 53/N`) — `InspectorPanel.js`. v1 shows the refund $ in the confirm prompt; v2 just says "partial refund".
...
- [x] **Period toggle on Treasury** (`v1-parity audit, commit 64/N`) — `CityTreasuryTab.js`. v1 has Today / Week / All-time. v2 shipped Today / Week / Month (capped at 30d since cash_transactions doesn't paginate cheaply past that). Resources doesn't need a period toggle — its data is per-tick instantaneous rates, not a time window.
...
- [x] **Help: click-to-expand rows + recipe period labels** (`v1-parity audit, commit 65/N`) — `HelpModal.js`. Benefit one-liner shipped in 48; click-to-expand cards + integer-ratio recipe periods shipped in 65.
