...
- [ ] **Inspector "Issues" section** — `InspectorPanel.js`. v1 renders a consolidated bulleted list of every reason a building isn't operational ("no road — place a road adjacent", "missing 5 lumber", "no workers"). v2 has just a single `Status` row.
