...
- [x] **Resources drilldown — flow breakdown** (`v1-parity audit, commit 50/N`) — `CityResourcesTab.js`. v1: click a row to expand "where it's going" (production sources, processor sinks, citizen drain, exports + per-partner flow). v2 shipped the flow breakdown; **per-partner trade table queued as a follow-up commit** (51).
