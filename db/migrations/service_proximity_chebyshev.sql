-- ─────────────────────────────────────────────────────────────────────
-- Service proximity → Chebyshev (2026-05-20).
--
-- Atlas: players intuit "school within 5 tiles" as a 5-tile square
-- around the school (king's-move / Chebyshev distance). The previous
-- check used Manhattan distance, so a townhouse 2 tiles right + 4 tiles
-- up from a school registered as 6 away — visually 4 tiles, gated as
-- 6 — and the upgrade silently refused. Bug filed by Jill (2026-05-18):
-- "Townhouses indicate there is not an operating school, but both have
-- schools within 5 tiles so should qualify to upgrade." She was right;
-- the school WAS within 5 tiles visually.
--
-- This migration swaps the metric in:
--   - has_well_access            (well coverage, range 4)
--   - _pp_evolve_housing         (school 5, temple 6, bathhouse 4)
--
-- Net effect: service coverage areas become squares (11×11 for school)
-- instead of diamonds (61-tile diamond). ~2.5× the covered area. Many
-- houses that were stuck just outside the Manhattan corner will now
-- upgrade naturally on the next tick.
--
-- Other Manhattan uses in the codebase are intentionally untouched
-- (boost adjacency, extractor adjacency, desirability falloff, crime
-- spread, food-near-road heuristic) — those model walking / influence
-- diffusion where Manhattan is the right metric.
-- ─────────────────────────────────────────────────────────────────────
...
AS $function$
  SELECT EXISTS (
    ...
    WHERE bt.key = 'well' AND b.status = 'active'
      ...
      AND GREATEST(ABS(b.x - p_x), ABS(b.y - p_y)) <= 4
  );
$function$;
...
BEGIN
  ...
  LOOP
    ...
    -- Service proximity uses Chebyshev (king's-move) distance: range 5
    -- school = 11×11 square around the school, matching what players
    -- visually estimate.
