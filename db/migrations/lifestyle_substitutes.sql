-- ─────────────────────────────────────────────────────────────────────
-- Lifestyle substitutes (2026-05-18).
--
-- Atlas: the bread requirement on housing is steep. Instead of bread
-- being the only acceptable good, players should also be able to
-- satisfy it with spices, caviar, or spirits — any combination.
--
-- Model: a separate substitutes table maps a primary lifestyle
-- demand resource (e.g. 'bread') to any number of acceptable
-- substitutes. The per-house pantry buffer stays keyed by the primary
-- ('bread'), but the refill phase pools from inventory of the primary
-- + every substitute (mirroring how the food buffer pools across all
-- is_food resources). The upgrade gate accepts any of them as
-- in-stock for the demand.
--
-- Net player effect: a house gated on bread can be sustained by any
-- mix of {bread, spices, caviar, spirits}. The buffer fills from
-- whichever is most abundant.
-- ─────────────────────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS public.lifestyle_substitutes (
  primary_key    text NOT NULL,
  substitute_key text NOT NULL,
  PRIMARY KEY (primary_key, substitute_key)
);

ALTER TABLE public.lifestyle_substitutes ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS lifestyle_substitutes_read ON public.lifestyle_substitutes;
CREATE POLICY lifestyle_substitutes_read ON public.lifestyle_substitutes FOR SELECT USING (true);
GRANT SELECT ON public.lifestyle_substitutes TO anon, authenticated;
...
-- ── Refill phase ────────────────────────────────────────────────────
-- Same external contract. For buffers whose primary has substitutes,
-- pool from inventory of primary ∪ substitutes and drain proportionally
-- across the pool (mirrors the 'food' branch).
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      SELECT EXISTS (
        SELECT 1 FROM public.lifestyle_substitutes ls
        WHERE ls.primary_key = v_resource.resource_key
      ) INTO v_has_subs;

      IF v_has_subs THEN
        -- Pool inventory across primary + every substitute.
        SELECT COALESCE(SUM(i.quantity), 0) INTO v_avail
        FROM public.inventories i
        WHERE i.player_id = p_uid
          AND (
            i.resource_key = v_resource.resource_key
...
-- ── Upgrade gate ────────────────────────────────────────────────────
-- _pp_evolve_housing's upgrade gate previously required inventory of
-- the EXACT demand resource. Now: a demand is satisfied by the primary
-- OR any substitute. Devolve gate (per-house buffer) is unchanged —
-- the buffer fills from substitutes via refill above, so the buffer
-- being non-empty correctly reflects "has acceptable substance".
...
BEGIN
  ...
  LOOP
    ...
    -- Upgrade gate now accepts substitutes: a demand is satisfied if
    -- the primary OR any of its substitutes is in stock.
