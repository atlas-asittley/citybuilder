-- ─────────────────────────────────────────────────────────────────────
-- Track the reason for each house's most recent devolve (2026-05-10).
--
-- Atlas: "for the inspector for the house, it should tell you the
-- reason the house downgraded the last time it downgraded. if it
-- didn't downgrade at all it doesn't need that part."
--
-- Adds three columns to `buildings`:
--   - last_devolve_reason text — short key like 'food', 'school',
--                                'lifestyle:furniture', 'desirability'
--   - last_devolve_at timestamptz — when it happened
--   - last_devolve_from_tier int — what tier it dropped FROM (the to
--                                  tier is just from_tier - 1)
--
-- NULL on all three = never devolved → inspector hides the section.
--
-- Updates `_pp_evolve_housing` to capture the first failing gate as
-- the reason whenever a devolve fires. The reason vocabulary matches
-- the existing `describeUpgradeBlocker` keys on the client so the
-- prose layer doesn't need new strings.
-- ─────────────────────────────────────────────────────────────────────

ALTER TABLE public.buildings
  ADD COLUMN IF NOT EXISTS last_devolve_reason text,
  ADD COLUMN IF NOT EXISTS last_devolve_at timestamptz,
  ADD COLUMN IF NOT EXISTS last_devolve_from_tier int;
...
BEGIN
  ...
  LOOP
    ...
    -- Find the first empty-buffer lifestyle resource (for reason
    -- attribution). NULL means no lifestyle gate is failing.
    ...
    IF v_should_devolve THEN
      -- Pick the most informative reason. Order matches the OR chain
      -- above so the first short-circuit gate wins. Lifestyle-good
      -- failure surfaces the specific resource (e.g., 'lifestyle:bread')
      -- since "missing furniture" is more actionable than "missing
      -- some lifestyle good."
...
-- Changelog entry — player-visible inspector addition.
