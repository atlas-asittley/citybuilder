-- ── process_production: include worker_capacity + workers_used in the
-- JSON response (2026-05-08) ──
-- The client (game.js) reads `data.worker_capacity` and
-- `data.workers_used` after every tick to populate
-- state.profile.worker_capacity / workers_used. The server has been
-- returning `worker_supply` (a different key) and never including
-- `workers_used` at all, so the client's checks
--   if (data.worker_capacity !== undefined) ...
--   if (data.workers_used !== undefined) ...
-- silently failed every tick. Result: state.profile.worker_capacity
-- only updated when a building was placed (place_building's response
-- uses worker_capacity correctly), so the topbar workers tile froze
-- between placements.
--
-- Atlas: "make sure workers employed and population are updated every
-- tick." This was the gap.
--
-- Fix: include both `worker_capacity` and `workers_used` in the
-- response. Keeping `worker_supply` as well so any other client code
-- expecting it stays working.
--
-- Also re-issuing the function unchanged otherwise (verbatim from
-- prior live source plus the JSON additions).
