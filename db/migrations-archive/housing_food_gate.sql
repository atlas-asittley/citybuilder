-- Housing food gate: tier 1+ requires "any food" in inventory.
--
-- New schema:
--   resources.is_food            boolean — marks a resource as food
--   housing_tier_config.needs_food boolean — tier requires food to evolve / stay
--
-- Today's foods: grain, flour, bread. After the food-pairing item lands,
-- berries / fish / vegetables / etc. just get is_food = true and the gate
-- works for them automatically — no further code change.
--
-- Strictness: presence-only check (any food > 0 → pass). NOT consumption.
-- Consumption arrives once every player has a paired food crop, since
-- non-grain industries can't make food locally yet. Until then, draining
-- food per-tick would punish stone/clay/timber players unfairly. Switching
-- to consumption later is a one-line change in the housing eval.
--
-- Apply: psql "$DB_URL" -f housing_food_gate.sql
...
-- Mark today's foods. Each step in the grain → flour → bread chain qualifies.
UPDATE public.resources SET is_food = true WHERE key IN ('grain', 'flour', 'bread');

-- Tier 1 (Mud Hut) and up require food. Shanty (tier 0) is the
-- subsistence floor — no food required so a player without trade access
-- still has a baseline.
UPDATE public.housing_tier_config SET needs_food = true WHERE tier >= 1;

-- ── 2. process_production: add v_has_food check to housing eval ──
...
BEGIN
  ...
  -- Services (multi-input feeding; populates v_operating_services + v_tavern_bonus)
  FOR v_building IN
  ...
  -- ── Food check (presence-only): any is_food resource > 0 in inventory ──
  -- This runs AFTER service feeding consumed inputs, so a player who used
  -- the last of their food on a tavern this tick correctly fails the food
  -- gate at housing eval.
  SELECT EXISTS (
