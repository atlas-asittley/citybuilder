-- ============================================================
-- City Builder - Tier 3 Production Chains Migration
-- ============================================================
-- Run AFTER the Clay Chain migration is in place.
-- Adds: Bread + Furniture resources, Bakery (common processor)
--        + Woodcarver (timber processor), trader prices, Black
...
INSERT INTO public.resources (key, name, kind, industry_key) VALUES
  ('bread',     'Bread',     'processed', 'common'),
  ('furniture', 'Furniture', 'processed', 'timber')
ON CONFLICT (key) DO NOTHING;

-- ────────────────────────────────────────────────────────────
-- 1b. EXTEND TIER CONSTRAINT
-- ────────────────────────────────────────────────────────────

ALTER TABLE public.building_types DROP CONSTRAINT IF EXISTS building_types_tier_check;
ALTER TABLE public.building_types ADD CONSTRAINT building_types_tier_check CHECK (tier = ANY (ARRAY[1, 2, 3]));

-- ────────────────────────────────────────────────────────────
-- 2. NEW BUILDING TYPES
-- ────────────────────────────────────────────────────────────

-- Bakery: processor, common (all players), Tier 3
--   3 workers, $400, consumes 1 flour/min → produces 0.5 bread/min
INSERT INTO public.building_types (
  ...
  'bakery', 'Bakery', 3, 'common', 'processor', 400, 3,
  'flour', 1, 'bread', 0.5,
  0
)
...
-- Woodcarver: processor, timber industry, Tier 3
--   2 workers, $450, consumes 0.5 lumber/min → produces 0.25 furniture/min
INSERT INTO public.building_types (
  key, name, tier, industry_key, category, build_cost, worker_cost,
  input_resource_key, input_rate, output_resource_key, output_rate,
  workers_provided
) VALUES (
  'woodcarver', 'Woodcarver', 3, 'timber', 'processor', 450, 2,
  'lumber', 0.5, 'furniture', 0.25,
  0
)
ON CONFLICT (key) DO UPDATE SET
  name = EXCLUDED.name,
...
-- ────────────────────────────────────────────────────────────
-- 3. TRADER PRICES FOR BREAD & FURNITURE
-- ────────────────────────────────────────────────────────────

-- River Traders: generalist
INSERT INTO public.trader_prices (trader_key, resource_key, buy_price, sell_price) VALUES
  ('river_traders', 'bread', 12, NULL),
  ('river_traders', 'furniture', 14, NULL)
...
-- Desert Caravan: premium buyer of luxury goods
INSERT INTO public.trader_prices (trader_key, resource_key, buy_price, sell_price) VALUES
  ('desert_caravan', 'bread', 15, NULL),
  ('desert_caravan', 'furniture', 18, NULL)
ON CONFLICT (trader_key, resource_key) DO UPDATE SET
  buy_price = EXCLUDED.buy_price, sell_price = EXCLUDED.sell_price, is_active = true;

-- Mountain Folk: bulk trader
INSERT INTO public.trader_prices (trader_key, resource_key, buy_price, sell_price) VALUES
  ('mountain_folk', 'bread', 10, NULL),
  ('mountain_folk', 'furniture', 12, NULL)
ON CONFLICT (trader_key, resource_key) DO UPDATE SET
  buy_price = EXCLUDED.buy_price, sell_price = EXCLUDED.sell_price, is_active = true;
...
-- ────────────────────────────────────────────────────────────
-- Add bread and furniture to the fixed-price emergency market.
-- Bread:     buy_from_player 8g, sell_to_player 22g
-- Furniture: buy_from_player 10g, sell_to_player 28g
...
CROSS JOIN (VALUES ('bread'), ('furniture')) AS r(key)
ON CONFLICT (player_id, resource_key) DO NOTHING;
