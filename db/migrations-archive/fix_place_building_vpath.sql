-- Patch: re-create place_building with v_path pre-initialized so
-- non-extractor placements (roads, housing, processors) don't hit
-- "record v_path is not yet assigned" when building the return JSON.
