-- Two new buildings:
--   well    (category='service'): cheap, no workers, no production. Required
--           within Manhattan distance 4 for housing to evolve past tier 0.
--   tax_man (category='tax'): periodically credits money to the player when
--           fully staffed. Future work will hook this into the happiness
--           system (tax man reduces happiness in its area), but for now it's
--           a flat tradeoff: pay for the building and workers, gain $/min.
--
-- Apply: psql "$DB_URL" -f well_and_tax_man.sql

-- ── 0. Allow new categories on building_types ──
...
ALTER TABLE public.building_types
  ...
  CHECK (category = ANY (ARRAY['extractor', 'processor', 'housing', 'road', 'service', 'tax']));

-- ── 1. Housing precondition: tier 1+ needs a well in range ──
ALTER TABLE public.housing_tier_config
  ADD COLUMN IF NOT EXISTS needs_well boolean NOT NULL DEFAULT false;

UPDATE public.housing_tier_config SET needs_well = true WHERE tier >= 1;

-- ── 2. Helper: tile near an active well owned by the same player? ──
-- Manhattan distance ≤ 4 = a 5×5 diamond, ~13 tiles per well.
CREATE OR REPLACE FUNCTION public.has_well_access(p_player_id uuid, p_x integer, p_y integer)
 RETURNS boolean
 LANGUAGE sql
 STABLE
AS $function$
...
GRANT EXECUTE ON FUNCTION public.has_well_access(uuid, integer, integer) TO authenticated;

-- ── 3. building_types: well + tax_man ──
INSERT INTO public.building_types
  (key, name, tier, industry_key, category, build_cost, worker_cost,
   input_resource_key, input_rate, output_resource_key, output_rate, is_active, workers_provided)
VALUES
  ('well', 'Well', 1, 'common', 'service', 50, 0,
   NULL, '0', NULL, '0', true, 0),
  ('tax_man', 'Tax Office', 2, 'common', 'tax', 300, 10,
   NULL, '0', NULL, '10', true, 0)
...
-- ── 4. process_production: housing evolution gates on well access AND
--      tax buildings credit money periodically ──
-- We rewrite process_production to layer in (a) the well check and
-- (b) the new tax-revenue loop. All other behavior is unchanged.
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      IF v_input_used > 0 THEN
        ...
        SET quantity = quantity - v_input_used
        WHERE player_id = v_uid AND resource_key = v_building.input_resource_key;
  ...
  -- ── TAX REVENUE ───────────────────────────────────────
  -- Each staffed tax building credits output_rate money per minute to
  -- the player. Future: combine with happiness so tax also pulls
  -- happiness down in an area.
  FOR v_building IN
  ...
  LOOP
    ...
    IF v_should_upgrade THEN
      ...
      SET housing_tier = housing_tier + 1, last_processed_at = v_now
      WHERE id = v_house.id;
  ...
  -- ── FINAL UPDATE (re-derive supply since housing tiers may have changed) ──
...
-- ── 5. place_building's worker calculation needs to include 'tax' too ──
-- Layer the tax category into the worker_supply / workers_needed math
-- so the topbar reads correctly the moment a tax building is placed.
