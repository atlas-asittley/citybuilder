-- All production buildings (extractors / food extractors / boosters /
-- processors / tax / services) now require road access to be staffed.
-- Previously extractors / food extractors / boosters were unconditionally
-- staffed (no road check), so they could operate from islands that
-- weren't reachable by walkers. Atlas wants every building to need road
-- access.
--
-- Apply: psql "$DB_URL" -f all_buildings_need_road.sql
